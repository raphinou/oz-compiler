...
If that record is a cons, i.e. a record with label '|' and with exactly two features with values 1 and 2, we need to issue a makeConsUnify(DestReg). If the arity is made of numbers only, i.e. we have the arity of a tuple, we need to issue a makeTupleUnify(k(Label) FeaturesList DestReg) instruction. Else we need to issue a makeRecordUnify(k(Arity) FeaturesList DestReg) instruction.
