...
\subsubsection{Records}
At this step, the only place we can find records are in the right hand side of unifications, i.e. the second features of fEq. If the arity is made of numbers only, i.e. we have the arity of a tuple, we need to issue a makeTupleUnify(k(Label) FeaturesList DestReg) instruction. If not, we need to issue a makeRecordUnify(k(Arity) FeaturesList DestReg) instruction. 
...
The featuresList has to be ordered according to the features. This is important, as the makeTupleUnify/makeRecordUnify instructions have to be followed by arrayFill instructions, one for each value, in the same order as the features passed to the instruction.
